ActiveAdmin.register Delivery do
  permit_params :deliveryType, :course, :courseValidity, :amount, :source, :purchasedAt, :name, :mobile, :address, :counselorName, :trackingNumber, :usb, :dongle, :delivered
  active_admin_import validate: true,
    headers_rewrites: { 'delivery type': :deliveryType, 'course': :course, 'course validity': :courseValidity, 'amount': :amount, 'source': :source, 'purchased at': :purchasedAt, 'name': :name, 'mobile': :mobile, 'address': :address, 'counselor name': :counselorName, 'tracking number': :trackingNumber, 'usb': :usb, 'dongle': :dongle, 'delivered': :delivered, 'created at': :createdAt, 'updated at': :updatedAt},
    template_object: ActiveAdminImport::Model.new(
        hint: "file will be imported with such header format: 'delivery_type','course','course_validity'",
        csv_headers: ["delivery type","course","course validity","amount","source","purchased at","name","mobile","address","counselor name","tracking number","usb","dongle","delivered","created at","updated at"]
    )
