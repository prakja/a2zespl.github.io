ActiveAdmin.register DuplicateQuestion do
  ...
  index do
    ...
    column "Question1" do |dq|
      raw('<a href=' + edit_admin_question_path(dq.questionId1) + " target='_blank'>#{dq.questionId1}</a><br />" + dq&.question1&.question)
    end
    column "Question2" do |dq|
      raw('<a href=' + edit_admin_question_path(dq.questionId2) + " target='_blank'>#{dq.questionId2}</a><br />" + dq&.question2&.question)
    end
    column :similarity
