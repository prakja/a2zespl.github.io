ActiveAdmin.register Test do
  ...
  member_action :mark_not_duplicate_practice_question, method: :post do
    NotDuplicateQuestion.create!(
      questionId1: params[:question_id1].to_i < params[:question_id2].to_i ? params[:question_id1].to_i : params[:question_id2].to_i,
      questionId2: params[:question_id1].to_i < params[:question_id2].to_i ? params[:question_id2].to_i : params[:question_id1].to_i
    )
    respond_to do |format|
      format.html {redirect_back fallback_location: duplicate_practice_questions_admin_test_path(resource), notice: "marked as not duplicate question so that we get only missing questions from this test"}
      format.js
    end
  end
