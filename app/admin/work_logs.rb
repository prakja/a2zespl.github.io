ActiveAdmin.register WorkLog do
  ...
  form do |f|
    ...
    f.object.date = Date.today.to_s if f.object.new_record? 
    f.inputs "Work Log" do
      render partial: 'tinymce', :locals => {new: f.object.new_record?}
      f.input :date, as: :date_picker, :input_html => { :placeholder => "DD-MM-YYYY", :min => Date.today - 15.days, :max => Date.today, :readonly => f.object.persisted? }, :hint => f.object.persisted? ? "Date can't be changed now!!" : "Please select date carefully as you won't be able to change it later"
