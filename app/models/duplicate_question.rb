class DuplicateQuestion < ApplicationRecord
  ...
  # assume that we want to keep quesiton 2 and remove qeustion 1
  def remove_q1_from_question_bank
    ActiveRecord::Base.connection.query('delete from "ChapterQuestion" where "questionId" = ' + self.questionId1.to_s + ' and "chapterId" in (select "chapterId" from "ChapterQuestion" where "questionId" in  (' + self.questionId1.to_s + ', ' + self.questionId2.to_s + ') group by "chapterId" having count(*) > 1);')
  end
